...
```bash
PYTHONPATH=src "$(git rev-parse --git-common-dir)/../.venv/bin/python" -m pytest -q \
...
```bash
ATLASSIAN_E2E=1 PYTHONPATH=src \
  "$(git rev-parse --git-common-dir)/../.venv/bin/python" -m pytest -q \
...
```bash
"$(git rev-parse --git-common-dir)/../.venv/bin/ruff" format --check .
PYTHONPATH=src "$(git rev-parse --git-common-dir)/../.venv/bin/python" -m pytest -q
"$(git rev-parse --git-common-dir)/../.venv/bin/ruff" check README.md pyproject.toml src tests docs
```
